...
# ╔═╡ 5a95b347-009d-4dc2-a5f9-e436020879fb
begin
	using GLMakie
	using PlutoUI
end

# ╔═╡ 1f353b62-1ecc-11f1-b522-db9084b9cc23
md"# Convolution Visualizer"

# ╔═╡ 7fb99f1a-606c-4013-8464-953b7794fb88
@bind t PlutoUI.Slider(-4.9:0.01:12.9, default = 0.0, show_value=true)

# ╔═╡ 217478e7-2871-4111-bfbf-41777b502ebc
begin

	# initial functions
	tau = range(-10,10,2001)
	rect1 = [abs(i) <= 1 ? 1 : 0 for i in tau]
	rect2 = [abs(i-4) <= 1 ? 1 : 0 for i in tau]
	tri = [abs(i-4) <= 2 ? 2 - abs(i-4) : 0 for i in tau]

	# flip rect2
	flipped_rect2 = [abs(i-(-4)) <= 1 ? 1 : 0 for i in tau]

	# shift
	shifted_flipped_rect2 = [abs(i-(-4+t)) <= 1 ? 1 : 0 for i in tau]

	# compute overlapped area
	overlapped_area = rect1 .* shifted_flipped_rect2
end;

# ╔═╡ d63e966d-5e0a-4c70-b75e-616ac7d9423f
begin
	fig1 = Figure()
	ax1 = Axis(fig1[1, 1])
	ax2 = Axis(fig1[2, 1])
	...
	lines!(ax2, tau, rect1)
	lines!(ax2, tau, flipped_rect2)

	lines!(ax3, tau, rect1)
	lines!(ax3, tau, shifted_flipped_rect2)

	lines!(ax4, tau, overlapped_area)
